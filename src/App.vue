...
//Lógica para integrar o MetaMask à aplicação
...
const findMetamaskAccount = async () =>{
  ...
  try{
    ...
    if(!ethereum){
      window.alert("Instale a extensão do MetaMask!")
      return null
    }

    window.alert("A extensão do MetaMask está instalada:", ethereum)
    ...
    if(accounts.length !== 0 ){
      ...
      console.log("Nenhuma conta MetaMask conectada :/")
      return null
    }
  }catch(error){
    console.error(error)
